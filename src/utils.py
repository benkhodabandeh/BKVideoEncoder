# utils.py
"""
General utility functions for Ben. Khodabandeh Video Encoder including path management,
...
def find_resource_path(resource_name: str) -> Optional[str]:
    ...
    logger.warning(f"Resource '{resource_name}' could not be found in standard locations or system PATH.")
    return None
...
def get_app_settings_path() -> str:
    ...
    documents_path = os.path.join(os.path.expanduser('~'), 'Documents', config.LOG_FOLDER_NAME)
...
def sanitize_filename_component(name: str, allow_spaces: bool = True, fallback: str = "Untitled") -> str:
...
def generate_output_path(base_dir: str, meta: Dict[str, str], container: str) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    try:
        ...
        sanitized_folder_name = sanitize_filename_component(folder_name, True, "Output_Folder")
        ...
        sanitized_filename = sanitize_filename_component(base_filename_str, True, "Output_Video")
...
def generate_stills_path(base_dir: str, meta: Dict[str, str]) -> str:
    ...
    sanitized_folder_name = sanitize_filename_component(folder_name, True, "Stills_Folder")
...
def get_aspect_aware_scale_filter(source_w: int, source_h: int, target_res_name: str, target_w_override: int = 0) -> Tuple[str, int, int]:
    if not all(isinstance(val, int) and val > 0 for val in [source_w, source_h]):
        return "", source_w, source_h
    if target_w_override > 0:
        target_w, target_h = target_w_override, int(target_w_override / (source_w / source_h)) 
    else:
        target_res = config.RESOLUTIONS.get(target_res_name)
        if not target_res: return "", source_w, source_h
        target_w, target_h = target_res['width'], target_res['height']
    if not target_w_override and (source_w <= target_w and source_h <= target_h):
        logger.info(f"Output resolution target ({target_res_name}) is not smaller than source. No scaling will be applied.")
        return "", source_w, source_h
    ...
    logger.info(f"Aspect-aware scale: {source_w}x{source_h} -> {out_w}x{out_h} (Target: {display_target})")
    return f"scale={out_w}:{out_h}:{config.SCALING_FILTER_FLAGS}", out_w, out_h

def get_aspect_ratio_crop_filter(source_w: int, source_h: int, aspect_ratio_str: str) -> Tuple[str, int, int]:
    try:
    ...
    if source_ar > target_ar:
        new_w, new_h, crop_x, crop_y = int(source_h * target_ar), source_h, (source_w - int(source_h * target_ar)) // 2, 0
    else:
        new_w, new_h, crop_x, crop_y = source_w, int(source_w / target_ar), 0, (source_h - int(source_w / target_ar)) // 2
...
def calculate_video_bitrate(pid: str, preset_config: Dict, source_info: Dict, job_item: Dict, out_w: int, out_h: int) -> int:
    quality_level = job_item.get('quality_level', config.Quality.DEFAULT_LEVEL)
    if pid == "THE_JOB":
        try: target_size_mb = float(job_item['target_mb_val'])
        except (ValueError, TypeError): target_size_mb = config.WORKFLOW_PRESETS['THE_JOB']['default_target_mb']
        duration_s, audio_bitrate_kbps = source_info.get('duration', 1), preset_config.get("audio_bitrate_kbps", 128)
        target_bits, audio_bits = target_size_mb * 1024 * 1024 * 8, audio_bitrate_kbps * 1000 * duration_s
        video_kbps = max(0, (target_bits - audio_bits)) / (duration_s * 1000)
        return int(max(preset_config.get('min_bitrate_kbps', 100), min(video_kbps, preset_config.get('max_bitrate_kbps', 50000))))
    ...
    if pid == "THE_GHOST":
        ...
        base_res_pixels = config.RESOLUTIONS['UHD']['width'] * config.RESOLUTIONS['UHD']['height']
...
def run_process(
    cmd: List[str], duration: Optional[float] = None, total_frames: Optional[int] = None,
    progress_callback: Optional[Callable[[Dict[str, Any]], None]] = None,
    process_description: str = "FFmpeg Process", process_holder: Optional[List[subprocess.Popen]] = None,
    cancel_flag_func: Optional[Callable[[], bool]] = None, cwd: Optional[str] = None
) -> Tuple[int, str, str]:
    cmd_str, progress_cmd = ' '.join(shlex.quote(c) for c in cmd), cmd
    logger.info(f"Running ({process_description}): {cmd_str}")
    if cwd: logger.info(f"Setting CWD for process: {cwd}")
    if '-progress' not in cmd: progress_cmd = cmd[:1] + ['-progress', 'pipe:1'] + cmd[1:]
    ...
    try:
        ...
        if platform.system() == "Windows":
            startupinfo, creationflags = subprocess.STARTUPINFO(), subprocess.CREATE_NO_WINDOW
            startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            startupinfo.wShowWindow = subprocess.SW_HIDE
        process = subprocess.Popen(progress_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='replace', startupinfo=startupinfo, creationflags=creationflags, cwd=cwd, bufsize=1)
        if process_holder is not None: process_holder.append(process)
        
        stdout_thread = threading.Thread(target=reader_thread, args=(process.stdout, output_queue), daemon=True)
        stderr_thread = threading.Thread(target=reader_thread, args=(process.stderr, output_queue), daemon=True)
        stdout_thread.start()
        stderr_thread.start()
        ...
        while finished_streams < 2:
            if cancel_flag_func and cancel_flag_func():
                if process.poll() is None:
                    logger.warning(f"Cancellation received for '{process_description}'. Terminating...")
                    process.terminate()
                break
            ...
            if progress_callback and (current_time - last_update_time > config.FFMPEG_PROGRESS_INTERVAL_SEC):
                last_update_time = current_time
                
                try: frame = int(progress_data.get('frame', 0))
                except (ValueError, TypeError): frame = 0
                
                if 'out_time_us' in progress_data:
                    out_time_s = _parse_time_to_seconds(progress_data.get('out_time_us')) / 1_000_000.0
                else:
                    out_time_s = _parse_time_to_seconds(progress_data.get('out_time'))

                if frame == 0 and out_time_s > 0 and duration and duration > 0 and total_frames and total_frames > 0:
                    frame = int(total_frames * (out_time_s / duration))

                progress_percent, progress_text = 0.0, "..."
                if total_frames and total_frames > 0:
                    progress_percent = min(100.0, (frame / total_frames) * 100) if frame > 0 else 0.0
                    progress_text = f"{frame}/{total_frames} ({progress_percent:.1f}%)"
                elif duration and duration > 0:
                    progress_percent = min(100.0, (out_time_s / duration) * 100) if out_time_s > 0 else 0.0
                    progress_text = f"{progress_percent:.1f}%"
                
                try: speed_val = float(progress_data.get('speed', '0.0x').replace('x', ''))
                except (ValueError, TypeError): speed_val = 0.0
                
                eta = (duration - out_time_s) / speed_val if speed_val > 0 and duration and out_time_s > 0 else 0
        ...
        while not output_queue.empty():
            line = output_queue.get_nowait()
            if line and '=' not in line: full_stderr.append(line)
        return_code = process.wait()
        stderr_str = "".join(full_stderr)
        if return_code != 0 and not (cancel_flag_func and cancel_flag_func()):
             logger.error(f"Process '{process_description}' failed with exit code {return_code}. Stderr: {stderr_str.strip()}")
        ...
        if process_holder is not None and process in process_holder:
            try: process_holder.remove(process)
...
def check_for_updates(current_version: str, settings: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    from packaging.version import parse as parse_version
    now_iso, last_check = datetime.now().isoformat(), settings.get('update_last_check', '2000-01-01')
    if (datetime.fromisoformat(now_iso) - datetime.fromisoformat(last_check)) < timedelta(days=7):
    ...
    try:
        req = urllib.request.Request(config.UPDATE_URL, headers={'User-Agent': f'{config.APP_NAME}/{current_version}'})
        with urllib.request.urlopen(req, timeout=5) as response:
            if response.status == 200:
                ...
                if latest_version:
                    ...
                    if parse_version(latest_version) > parse_version(current_version) and latest_version != skipped_version:
                        logger.info(f"Update found! New version: {latest_version}")
                        return data
                    else:
                        logger.info(f"Application is up to date or version {latest_version} was skipped.")
            else:
                logger.warning(f"Update check failed with status code: {response.status}")
    except urllib.error.URLError as e:
        logger.warning(f"Could not check for updates due to a network error. Details: {e.reason}")
    except Exception as e:
...
def get_perf_log_path() -> str:
    doc_path = os.path.join(os.path.expanduser('~'), 'Documents', config.LOG_FOLDER_NAME)
...
def write_perf_log(perf_data: Dict[str, Any]):
    ...
    try:
        with open(get_perf_log_path(), 'w', encoding='utf-8') as f: json.dump(records, f, indent=4)
    except IOError as e:
...
def get_time_estimate(job_item: Dict[str, Any], out_w: int, out_h: int) -> float:
    ...
    matches = []
    for rec in records:
        if rec.get('preset_id') == preset_id and rec.get('encoder_preset') == encoder_preset:
            diff = abs(rec.get('pixels', 0) - pixels)
            matches.append({'diff': diff, 'record': rec})
    if not matches:
        logger.warning(f"No performance history found for preset '{preset_id}' with speed '{encoder_preset}'.")
        return 0.0
    matches.sort(key=lambda x: x['diff'])
    top_matches = matches[:5]
    average_time_per_second = sum(m['record']['time_per_second'] for m in top_matches) / len(top_matches)
    estimated_time = average_time_per_second * duration
    logger.info(f"Found {len(top_matches)} similar past encodes. Averaged estimate: {estimated_time:.1f}s")
