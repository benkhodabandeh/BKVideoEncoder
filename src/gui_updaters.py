# gui_updaters.py
"""
Functions for updating GUI elements in the Ben. Khodabandeh Video Encoder application.
...
def handle_status_update(app: 'App', message: str) -> None:
    if status_label := app._get_widget('status_label'):
        max_len = 150
        display_message = message if len(message) <= max_len else f"{message[:max_len-3]}..."
...
def handle_encoding_progress_update(app: 'App', progress_data: Dict[str, Any]) -> None:
    ...
    status_text = f"Processing {job_num_str}: {job_desc} @ {speed_str} [{progress_text_str}]"
...
def display_welcome_screen(app: 'App') -> None:
    if not (preview_canvas := app._get_widget('preview_canvas')): return
    ...
    if app.logo_photo_image:
        preview_canvas.create_image(canvas_w / 2, y_pos, anchor="center", image=app.logo_photo_image)
        y_pos += app.logo_photo_image.height() / 2 + 30
    else:
        preview_canvas.create_text(canvas_w / 2, y_pos, text=config.APP_NAME, fill=config.Theme.TEXT_PRIMARY, font=config.Theme.FONT_H1, anchor="center")
        y_pos += 40
    
    preview_canvas.create_text(canvas_w / 2, y_pos, text=f"Load evidence to begin.", fill=config.Theme.TEXT_SECONDARY, font=config.Theme.FONT_BODY, anchor="center")
...
def display_preview_image(app: 'App', image_path: Optional[str]) -> None:
    ...
    try:
        ...
        with Image.open(image_path) as pil_image:
            ...
            if crop_mode == 'Auto-Detect': crop_filter_str = info.get('auto_crop_string')
            elif crop_mode == 'Custom': crop_filter_str = app.custom_crop_var.get()
            elif crop_mode != 'None': crop_filter_str, _, _ = utils.get_aspect_ratio_crop_filter(pil_image.width, pil_image.height, crop_mode)
            ...
            if crop_filter_str and 'crop=' in crop_filter_str:
                try:
                    ...
                    logger.warning(f"Invalid crop string for preview '{crop_filter_str}': {e}")

            if colors := app.preview_stills_colors.get(image_path):
                if img_with_palette := analysis.add_palette_to_still(img_to_display, colors):
                    img_to_display = img_with_palette

            img_w, img_h = img_to_display.size
            if img_w <= 0 or img_h <= 0: return

            scale = min((canvas_w - config.Theme.PADDING*2) / img_w, (canvas_h - config.Theme.PADDING*2) / img_h)
            new_w, new_h = int(img_w * scale), int(img_h * scale)

            if new_w > 0 and new_h > 0:
                resized_img = img_to_display.resize((new_w, new_h), Image.Resampling.LANCZOS)
                ...
                preview_canvas.create_image(img_x, img_y, anchor="nw", image=photo_image)
    ...
    for button_key, preset_set in preset_sets_map.items():
        for pid, button_ref in app.widget_refs.get(button_key, {}).items():
            if button := app._get_widget(button_ref):
                ...
                if is_active:
                    button.configure(fg_color=config.Theme.SECONDARY, hover_color=config.Theme.SECONDARY_HOVER, text_color=config.Theme.TEXT_PRIMARY)
                else:
                    button.configure(fg_color=config.Theme.SURFACE_LIGHT, hover_color=config.Theme.SECONDARY, text_color=config.Theme.TEXT_SECONDARY)
...
def _update_slider_widgets(app: 'App', slider_key: str, label_key: str, options: list, current_value: Any):
...
def update_source_slider(app: 'App'):
    ...
    _update_slider_widgets(app, 'source_material_slider', 'source_material_label', options, current_val)
...
def update_quality_slider(app: 'App'):
    ...
    try:
        ...
        _update_slider_widgets(app, 'quality_level_slider', 'quality_level_label', options, current_idx)
    except ValueError:
        pass
...
def handle_set_video_info(app: 'App', video_info: Optional[Dict[str, Any]]) -> None:
    ...
    if video_info:
        ...
        if load_btn := app._get_widget('load_button'):
            load_btn.configure(fg_color=config.Theme.SUCCESS, text="Evidence Loaded") # Use success color
        if details_label := app._get_widget('source_details_label'):
            details_label.configure(text=video_info.get('source_details_text', 'N/A'), text_color=config.Theme.TEXT_PRIMARY)
        ...
        if downmix_check := app._get_widget('downmix_check'):
            ...
            if not is_multichannel: app.is_downmix_enabled.set(False)
    else:
...
def handle_set_preview_stills(app: 'App', paths_colors_tuple: Tuple[List[str], Dict[str, List[str]]]) -> None:
...
def handle_update_single_preview(app: 'App', index_path_colors_tuple: Tuple[int, str, Optional[List[str]]]) -> None:
...
def handle_update_all_preview_colors(app: 'App', colors_map: Dict[str, List[str]]):
    ...
    if app.current_preview_index >= 0 and len(app.preview_stills_paths) > app.current_preview_index:
        display_preview_image(app, app.preview_stills_paths[app.current_preview_index])

def show_task_complete_summary(app: 'App', jobs: int, outputs: int, stills: int, elapsed_time: float, estimated_time: float, output_path: str):
    details = f"• Outputs Created: {outputs} video file(s), {stills} still image(s)."
    if app.winfo_exists():
        ui_components.CompletionDialog(app, "The Job is Done", details, output_path, elapsed_time, estimated_time)

def update_estimates_display(app: 'App', bitrate_kbits: float, size_mb: float, time_s: float, crf_val: Optional[str]):
    labels = app.widget_refs.get('estimates_labels', {})
    if size_label := app._get_widget(labels.get('size')):
        if size_mb == -1: size_text = "Est. Size: N/A (CRF)"
        elif size_mb == -2: size_text = "Est. Size: N/A (Mixed)"
        elif size_mb > 0: size_text = f"Est. Size: {size_mb:.1f} MB"
        else: size_text = "Est. Size: --"
        size_label.configure(text=size_text)
    if bitrate_label := app._get_widget(labels.get('bitrate')):
        if crf_val:
            bitrate_text = f"Quality: CRF {crf_val}"
        elif size_mb == -2: bitrate_text = "Bitrate: N/A (Mixed)"
        elif bitrate_kbits > 0: bitrate_text = f"Bitrate: {bitrate_kbits:.0f} kbps"
        else: bitrate_text = "Bitrate: --"
        bitrate_label.configure(text=bitrate_text)
    if time_label := app._get_widget(labels.get('time')):
        time_text = f"~Est. Time: {utils._format_eta(time_s)}" if time_s > 0 else "~Est. Time: --:--:--"
