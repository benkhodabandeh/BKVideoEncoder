...
class App(ctk.CTk):
    ...
    def _update_estimates(self):
        ...
        for pid in all_presets:
            ...
            if pid == "THE_JOB":
                try:
                    total_size += float(self.target_mb_var.get())
                except (ValueError, TypeError):
                    total_size += config.WORKFLOW_PRESETS["THE_JOB"][
                        "default_target_mb"
                    ]
            else:
                ...
                total_size += (
                    (
                        ...
                        * self.input_file_info.get("duration", 0)
                    )
                    / 8
                    / 1024
                )
        ...
        for job in job_list:
            ...
            if not job_metadata.get("title") or not job_metadata.get("year"):
                if not messagebox.askyesno(
                    ...
                    return False
            if job_metadata.get("year") and not re.fullmatch(
                r"\d{4}", job_metadata.get("year", "")
            ):
                self._show_error(
                    "Invalid Metadata",
                    f"The job for '{job_title}' has an invalid year. It must be a 4-digit number (e.g., 2024).",
                )
                return False
        return True
