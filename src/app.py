# app.py
"""
Main application class and entry point for the Ben. Khodabandeh Video Encoder GUI.
...
class App(ctk.CTk):
    """Main application class for the Ben. Khodabandeh Video Encoder GUI."""
    def __init__(self):
        super().__init__()
    ...
    def _initialize_state(self):
        ...
        self.metadata: Dict[str, str] = {field: "" for field in config.METADATA_USER_FIELDS}
        ...
        self.target_mb_var = ctk.StringVar(value=str(config.WORKFLOW_PRESETS["THE_JOB"]["default_target_mb"]))
    ...
    def _check_essential_dependencies(self):
        if not utils.get_ffmpeg_path() or not utils.get_ffprobe_path():
            missing = [name for name, path in [(config.FFMPEG_EXE, utils.get_ffmpeg_path()), (config.FFPROBE_EXE, utils.get_ffprobe_path())] if not path]
            self._show_critical_error_and_exit("Dependency Error", f"Critical Error: Cannot find {' and '.join(missing)}.\n\nPlease ensure FFmpeg is in a 'bin' folder next to the application or in the system PATH.\n\nApplication will now exit.")
    ...
    def _load_assets(self):
        ...
        if analysis.HAS_PILLOW and (logo_path := utils.find_resource_path(config.LOGO_FILENAME)):
            if os.path.exists(logo_path):
                try:
                    with analysis.Image.open(logo_path) as logo_img:
                        logo_img.thumbnail((200, 200), analysis.Image.Resampling.LANCZOS)
    ...
    def _create_layout(self):
        ...
        left_panel_container = ctk.CTkFrame(main_frame, fg_color=config.Theme.BACKGROUND)
        left_panel_container.grid(row=0, column=0, padx=(padding, padding//2), pady=(0, padding), sticky="nsew")
        gui_panels.create_settings_panel(self, left_panel_container)

        right_panel = ctk.CTkFrame(main_frame, fg_color="transparent")
        right_panel.grid(row=0, column=1, padx=(padding//2, padding), pady=(0, padding), sticky="nsew")
        ...
        status_frame = ctk.CTkFrame(self, fg_color=config.Theme.SURFACE, corner_radius=0, height=40)
    ...
    def _initialize_async(self):
        ...
        if not self.app_temp_dir:
            self._show_critical_error_and_exit("Critical Error", "Could not create a temporary directory.")
    ...
    def _run_update_check(self):
        if update_info := utils.check_for_updates(config.APP_VERSION, self.app_settings):
    ...
    def _start_processing_task(self, description: str, task_func: Callable, args_for_task: tuple = (), is_develop_task: bool = False):
        if self.is_processing:
            self._show_warning("Busy", f"Already processing: {self.processing_task_description}")
            return
        self.is_processing, self.is_preparing, self.processing_task_description, self.cancel_requested = True, is_develop_task, description, False
        ...
        def task_wrapper(app_ref, *task_args):
            ...
            try:
                ...
                if app_instance:
                    app_instance.ui_update_queue.put(lambda: gui_updaters.handle_status_update(app_instance, f"Task Cancelled: {description}"))
            except Exception as e:
                logger.exception(f"Unhandled exception in background task '{description}':")
                if app_instance:
                    app_instance.ui_update_queue.put(lambda err=e: self._show_error(f"Task Error: {description}", f"Task failed unexpectedly:\n\n{err}"))
            finally:
                if app_instance and app_instance.winfo_exists():
                    app_instance.ui_update_queue.put(app_instance._finish_processing_task)
        threading.Thread(target=task_wrapper, args=(weakref.ref(self), *args_for_task), daemon=True).start()

    def _finish_processing_task(self):
        was_develop_run = self.is_develop_run
    ...
    def _update_ui_state(self):
        ...
        has_presets = bool(self.selected_standard_presets or self.selected_workflow_presets or self.selected_fast_presets)
        
        can_add_to_queue = can_interact and has_presets
        can_develop = is_idle and (bool(self.batch_queue) or (can_interact and has_presets))

        states = {
            'load_button': "normal" if is_idle else "disabled", 'dest_button': "normal" if is_idle else "disabled",
            'qc_button': "normal" if is_idle else "disabled", 'add_queue_button': "normal" if can_add_to_queue else "disabled",
            'develop_button': "normal" if can_develop else "disabled",
            'cancel_button': "normal" if not is_idle and not self.cancel_requested else "disabled",
            'prev_button': "normal" if is_loaded and len(self.preview_stills_paths) > 1 else "disabled",
            'next_button': "normal" if is_loaded and len(self.preview_stills_paths) > 1 else "disabled",
            'reload_button': "normal" if is_loaded and is_idle else "disabled",
            'mux_button': "normal" if is_idle else "disabled", 'audio_only_button': "normal" if is_idle else "disabled",
        }
        for name, state in states.items(): self._set_widget_state(name, state)

        for panel_key in ['meta_entries', 'preset_buttons', 'workflow_buttons', 'fast_preset_buttons', 
                          'source_material_slider', 'quality_level_slider', 'downmix_check', 'stills_check', 
                          'crop_menu', 'custom_crop_entry']:
            ...
            if isinstance(widget_or_dict, dict):
                [self._set_widget_state(w, state_to_set) for w in widget_or_dict.values()]
            else:
                self._set_widget_state(widget_or_dict, state_to_set)

        if not self.is_processing and not can_interact:
            self.ui_update_queue.put(lambda: gui_updaters.update_estimates_display(self, 0, 0, 0, None))

    def _update_destination_label(self):
        if dest_label := self._get_widget('dest_label'):
            path = self.destination_path or (os.path.dirname(self.input_file_original) if self.input_file_original else "[Source Directory]")
            dest_label.configure(text=f"Drop-off: {path}")

    def _update_estimates(self):
        if not self.is_video_loaded or not (self.selected_standard_presets or self.selected_workflow_presets or self.selected_fast_presets):
        ...
        all_presets = list(self.selected_standard_presets) + list(self.selected_workflow_presets) + list(self.selected_fast_presets)
        
        crf_presets_selected = [pid for pid in all_presets if (p := (config.STANDARD_PRESETS.get(pid) or config.WORKFLOW_PRESETS.get(pid) or config.FAST_PRESETS.get(pid))) and 'crf' in p.get('rate_control_mode')]
        if len(crf_presets_selected) == 1 and len(all_presets) == 1:
            pid = crf_presets_selected[0]
            preset_conf = config.STANDARD_PRESETS.get(pid) or config.WORKFLOW_PRESETS.get(pid) or config.FAST_PRESETS.get(pid)
            crf_val = preset_conf.get('crf_levels', {}).get(self.quality_level_var.get())

        for pid in all_presets:
            preset_conf = config.STANDARD_PRESETS.get(pid) or config.WORKFLOW_PRESETS.get(pid) or config.FAST_PRESETS.get(pid)
            if not preset_conf: continue

            # *** FIX: Use a deep copy to prevent state pollution between loop iterations ***
            ...
            total_time += utils.get_time_estimate(job_item, out_w, out_h)
            
            if 'crf' in preset_conf.get('rate_control_mode'):
                is_crf = True
                continue
            
            video_kbps = utils.calculate_video_bitrate(pid, preset_conf, self.input_file_info, job_item, out_w, out_h)
            total_bitrate += video_kbps
            
            if pid == "THE_JOB":
                try: total_size += float(self.target_mb_var.get())
                except (ValueError, TypeError): total_size += config.WORKFLOW_PRESETS['THE_JOB']['default_target_mb']
            else:
                audio_kbps = preset_conf.get("audio_bitrate_kbps", utils.get_audio_bitrate_from_options(preset_conf.get("audio_options", ""), 128))
                total_size += ((video_kbps + audio_kbps) * self.input_file_info.get('duration', 0)) / 8 / 1024
                
        if is_crf: 
            total_size = -2 if total_size > 0 else -1
        
        self.active_bitrate_kbits, self.estimated_size_mb, self.estimated_time_s = total_bitrate, total_size, total_time
        gui_updaters.update_estimates_display(self, self.active_bitrate_kbits, self.estimated_size_mb, self.estimated_time_s, crf_val)

    def _pre_flight_checks(self, job_list: List[Dict[str, Any]]) -> bool:
        if not job_list:
            self._show_error("Queue Empty", "There are no jobs in the queue to develop.")
            return False

        for job in job_list:
            job_metadata, job_title = job.get('metadata', {}), job.get('metadata', {}).get('title') or os.path.basename(job.get('input_file', '...'))
            if not job_metadata.get('title') or not job_metadata.get('year'):
                if not messagebox.askyesno("Missing Metadata", f"The job for '{job_title}' has a missing Title and/or Year.\n\nProceed anyway?", icon='question', parent=self):
                    return False
            if job_metadata.get('year') and not re.fullmatch(r"\d{4}", job_metadata.get("year", "")):
                self._show_error("Invalid Metadata", f"The job for '{job_title}' has an invalid year. It must be a 4-digit number (e.g., 2024).")
                return False
        return True

    def on_closing(self):
        if self.is_processing and not messagebox.askyesno("Confirm Exit", "A task is running. Are you sure you want to exit?", icon='warning', parent=self):
            return
        if self.is_processing: self.cancel_requested = True
        self._cleanup_before_exit()
        self.destroy()

    def _cleanup_before_exit(self):
        if self.qc_window and self.qc_window.winfo_exists(): self.qc_window.destroy()
        if self.app_temp_dir: utils.cleanup_temp_directory(self.app_temp_dir)
    ...
    def _get_widget(self, ref: Any) -> Optional[ctk.CTkBaseClass]:
        widget = ref() if isinstance(ref, weakref.ReferenceType) else self.widget_refs.get(ref) if isinstance(ref, str) else ref
        widget = widget() if isinstance(widget, weakref.ReferenceType) else widget
        if isinstance(widget, (ctk.CTkBaseClass, tk.Widget)) and widget.winfo_exists(): return widget
        return None

    def _set_widget_state(self, ref: Any, state: str):
        if (widget := self._get_widget(ref)) and hasattr(widget, 'configure') and widget.cget('state') != state:
    ...
    def _show_update_prompt(self, update_info: Dict[str, Any]):
        ...
        if choice == "skip":
            self.app_settings['update_skipped_version'] = update_info.get('latest_version')
        utils.save_app_settings(self.app_settings)
...
if __name__ == "__main__":
    try:
        ...
        logging.critical("A fatal error occurred during application startup.", exc_info=True)
        root = tk.Tk()
        root.withdraw()
