# analysis.py
"""
Functions for video analysis for Ben. Khodabandeh Video Encoder including:
...
try:
    ...
    logger.debug(f"Pillow library found (v{PILLOW_VERSION}). Palette and image features enabled.")
except ImportError:
    HAS_PILLOW = False
    if not hasattr(logging, '_pillow_warning_logged'):
        logging.warning("Pillow library not found. Palette and advanced image features will be disabled.")
        logging._pillow_warning_logged = True
    class Image:
        @staticmethod
...
def get_source_details_text(info: Dict[str, Any]) -> str:
    ...
    pix_fmt, color_transfer = v_stream.get('pix_fmt', ''), v_stream.get('color_transfer', '')
    hdr_info = "HDR" if "pq" in color_transfer or "hlg" in color_transfer else "SDR"
    video_part = f"{v_codec} / {_get_chroma_subsampling(pix_fmt)} / {_get_video_bit_depth(v_stream)}-bit ({hdr_info})"
    audio_part = "No Audio"
    if a_stream:
        channels, sample_rate = a_stream.get('channels', 0), int(a_stream.get('sample_rate', 0))
        channel_layout = a_stream.get('channel_layout', f"{channels}ch" if channels else "").title()
        sample_rate_str = f"{sample_rate / 1000:.1f}kHz".replace(".0", "") if sample_rate > 0 else "N/A"
...
def get_video_info(filepath: str) -> Optional[Dict[str, Any]]:
    if not (probe_path := utils.get_ffprobe_path()):
        ...
        return None
    cmd = [probe_path, "-v", "error", "-show_format", "-show_streams", "-print_format", "json", filepath]
    ...
    if ret_code != 0:
        logger.error(f"ffprobe failed for '{os.path.basename(filepath)}'. Exit: {ret_code}.\n{stderr.strip()}")
        return None
    try:
        ...
        duration = utils._parse_time_to_seconds(data.get("format", {}).get("duration")) or utils._parse_time_to_seconds(video_stream.get("duration"))
        ...
        result = {"duration": duration, "width": width, "height": height, "nb_frames": nb_frames,
                  "bit_rate": int(data.get("format", {}).get("bit_rate", 0)), "frame_rate": frame_rate,
                  "has_audio": bool(audio_stream), "video_stream": video_stream, "audio_stream": audio_stream}
        result["source_details_text"] = get_source_details_text(result)
        logger.info(f"Video Info: Duration={duration:.2f}s, Res={width}x{height}, Frames={nb_frames}")
        return result
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        logger.error(f"Error processing ffprobe JSON for {os.path.basename(filepath)}: {e}", exc_info=True)
        return None

def detect_crop(filepath: str, duration: float, source_w: int, source_h: int, detect_duration: int = config.CROP_DETECT_DURATION) -> Optional[str]:
    ...
    if analyze_time <= 0:
        ...
        return ""
    cmd = [ffmpeg_path, "-nostdin", "-hide_banner", "-i", filepath, "-t", f"{analyze_time:.3f}", "-vf", "cropdetect", "-f", "null", "-"]
    ...
    if source_w > 0 and source_h > 0 and int(w_str) == source_w and int(h_str) == source_h:
        logger.info(f"Detected crop '{most_common_param}' matches source resolution; no crop needed.")
        return ""
...
def analyze_source_complexity(info: Dict[str, Any]) -> str:
    pixels, framerate, bitrate = info.get('width', 0) * info.get('height', 0), info.get('frame_rate', 0), info.get('bit_rate', 0)
    ...
    if bpp <= config.SOURCE_COMPLEXITY_THRESHOLDS['clean']:
        ...
        suggestion = 'modern'
    logger.info(f"Source complexity analyzed (BPP: {bpp:.4f}). Suggested material: '{suggestion}'.")
    return suggestion
...
def _extract_still(input_filepath: str, timestamp: float, output_filepath: str, is_png: bool) -> bool:
    if not (ffmpeg_path := utils.get_ffmpeg_path()):
        return False
    logger.debug(f"Extracting still at {timestamp:.3f}s to {os.path.basename(output_filepath)}")
    try:
        os.makedirs(os.path.dirname(output_filepath), exist_ok=True)
    except OSError as e:
        logger.error(f"Cannot create directory for still '{output_filepath}': {e}")
        return False
    cmd = [ffmpeg_path, "-nostdin", "-hide_banner", "-ss", f"{timestamp:.6f}", "-i", input_filepath, "-vframes", "1"]
    # For truly uncompressed, pixel-perfect stills, just use the PNG codec without compression flags.
    if is_png:
        cmd.extend(["-c:v", "png"])
    else:
        cmd.extend(["-c:v", "mjpeg", "-q:v", str(config.PREVIEW_STILL_QUALITY), "-pix_fmt", "yuvj420p"])
    cmd.extend(["-y", output_filepath])
    ret_code, _, _ = utils.run_quick_process(cmd, "Still Extraction")
    return ret_code == 0 and os.path.exists(output_filepath) and os.path.getsize(output_filepath) > 100

def generate_stills_parallel(
    input_filepath: str, duration: float, num_stills: int, output_dir: str,
    is_png: bool, prefix: str, cancel_flag_func: Callable[[], bool],
    output_filename_base: Optional[str] = None, timestamps_to_use: Optional[List[float]] = None
) -> List[str]:
    """Generates multiple still images in parallel, with optional custom naming and timestamps."""
    timestamps = timestamps_to_use if timestamps_to_use is not None else _get_random_timestamps(duration, num_stills)
    if not timestamps:
        logger.error("Failed to generate timestamps for still extraction.")
        return []
    generated_files: List[Optional[str]] = [None] * len(timestamps)
    file_ext = config.FINAL_STILL_EXTENSION if is_png else config.PREVIEW_STILL_EXTENSION
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        ...
        for i, ts in enumerate(timestamps):
            if cancel_flag_func():
                break
            filename = f"{output_filename_base}_{i+1:02d}{file_ext}" if output_filename_base else f"{prefix}_still_{i:02d}{file_ext}"
            output_path = os.path.join(output_dir, filename)
            future = executor.submit(_extract_still, input_filepath, ts, output_path, is_png)
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            ...
            try:
                ...
                logger.error(f"Still extraction for {os.path.basename(path)} generated an exception: {exc}")
    successful_files = [p for p in generated_files if p]
    logger.info(f"Successfully generated {len(successful_files)}/{len(timestamps)} stills.")
    return successful_files

def generate_single_still(input_filepath: str, duration: float, index_to_replace: int, output_dir: str) -> Optional[Tuple[str, float]]:
    ...
    output_filepath = os.path.join(output_dir, f"preview_still_{index_to_replace:02d}{config.PREVIEW_STILL_EXTENSION}")
...
def analyze_image_colors(image_path: str, num_colors: int) -> Optional[List[str]]:
    ...
    try:
        with Image.open(image_path) as img:
            ...
            quantized_img = img_rgb.quantize(colors=num_colors, method=Image.Quantize.FASTOCTREE)
            palette_rgb_flat, color_counts = quantized_img.getpalette(), quantized_img.getcolors()
        ...
        logger.error(f"Error analyzing image colors in '{os.path.basename(image_path)}': {e}", exc_info=False)
        return None

def analyze_images_in_parallel(image_paths: List[str], cancel_flag_func: Callable) -> Dict[str, List[str]]:
    """Analyzes colors for multiple images in parallel."""
    colors_map: Dict[str, List[str]] = {}
    if not HAS_PILLOW:
        return colors_map
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        future_to_path = {executor.submit(analyze_image_colors, path, config.NUM_PALETTE_COLORS): path for path in image_paths}
...
def add_palette_to_still(image_or_path: Any, colors: List[str]) -> Optional[Image.Image]:
    ...
    try:
        ...
        for i, color_hex in enumerate(colors):
            draw.rectangle([i * block_w, 0, (i + 1) * block_w, palette_h], fill=color_hex, outline=color_hex)
