...
Ben. Khodabandeh Video Encoder. This module consolidates logic for standard presets and
...
try:
    ...
    if not getattr(logging, '_pyscene_warning_logged', False):
        logger.warning("PySceneDetect library not found. Falling back to FFmpeg's internal scene detection.")
        logging._pyscene_warning_logged = True
...
def get_scenecut_qp_filepath(job: Dict[str, Any], threshold: float, cancel_flag_func: Callable[[], bool]) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.info(f"PySceneDetect: Analyzing '{os.path.basename(video_path)}' with threshold {threshold}...")
        ...
        logger.info(f"PySceneDetect: Found {len(scene_list)} scenes. Saving QP file to {qp_filepath}")
...
def build_video_filter_chain(job: Dict[str, Any], preset_conf: Dict[str, Any]) -> Tuple[str, int, int]:
    ...
    if crop_str_from_job:
        try:
            full_crop_str = crop_str_from_job if 'crop=' in crop_str_from_job else f"crop={crop_str_from_job}"
            ...
            logger.warning(f"Invalid or incomplete custom crop string '{crop_str_from_job}', ignoring. Error: {e}")

    elif crop_mode not in ['None', 'Custom', 'Auto-Detect']:
        crop_filter, new_w, new_h = utils.get_aspect_ratio_crop_filter(source_w, source_h, crop_mode)
        if crop_filter:
    ...
    if is_downscaling:
        if quality_level != 'Prime Cut':
            if denoise_filter := preset_conf.get('size_denoise_filter') or preset_conf.get('denoise_filter'):
                v_filters.append(denoise_filter)
                logger.info(f"Applying denoiser before scaling (downscaling detected): {denoise_filter}")
            if sharpen_filter := preset_conf.get('size_sharpen_filter') or preset_conf.get('sharpen_filter'):
                v_filters.append(sharpen_filter)
                logger.info(f"Applying sharpener before scaling (downscaling detected): {sharpen_filter}")
    else:
        logger.info("No downscaling detected; filters will be applied after scaling if needed.")

    scale_filter, out_w, out_h = utils.get_aspect_aware_scale_filter(source_w, source_h, target_res_name, target_w_override)
    if scale_filter:
    ...
    if not is_downscaling:
        ...
        logger.info("'Prime Cut' quality or downscaling handled; no additional post-scale filters applied.")
...
def _build_audio_filter_chain(job: Dict[str, Any]) -> str:
    if source_sr := job['input_file_info'].get('audio_stream', {}).get('sample_rate', 0):
        if int(source_sr) != 48000:
            logger.info(f"Resampling audio from {source_sr}Hz to 48kHz.")
            return config.AUDIO_RESAMPLE_FILTER
    return ""

def _build_pass_command(job: Dict, pass_num: int, total_passes: int, video_opts: List[str], audio_opts: List[str], vf_str: str, af_str: str) -> List[str]:
    ...
    if is_final_pass:
        if v_stream := info.get('video_stream', {}):
            for key, flag in [('color_space', '-colorspace'), ('color_primaries', '-color_primaries'), ('color_trc', '-color_transfer')]:
        ...
        if info.get('has_audio'):
            ...
            if job['preset_id'] in ['THE_HEIST', 'THE_JOB']:
                logger.info(f"{job['preset_id']} preset: Forcing single stereo audio track.")
                ...
                if job.get('is_downmix_enabled') and info.get('audio_stream', {}).get('channels', 0) > 2:
                    acodec = job['preset_conf'].get('audio_codec') or config.AUDIO_CODEC
                    downmix_opts = ['-map', '0:a:0?', '-c:a:1', acodec, '-b:a:1', '192k', '-ac:1', '2']
                    if af_str:
...
def _execute_encode(job: Dict, total_passes: int, video_opts_base: List[str], audio_opts: List[str], vf_str: str, af_str: str, progress_callback: Callable, process_holder: List, cancel_flag_func: Callable) -> Tuple[bool, float]:
    info = job['input_file_info']
    ...
    for pass_num in range(1, total_passes + 1):
        ...
        if total_passes > 1:
            passlogfile = os.path.join(job['temp_dir'], f"{config.PASSLOG_FILENAME_BASE}_{job['job_id']}")
            if job['preset_conf']['video_codec'] == 'libx265':
        ...
        cmd = _build_pass_command(job, pass_num, total_passes, video_opts, audio_opts, vf_str, af_str)

        def pass_progress_wrapper(p_data):
            ...
            p_data['overall_progress'] = base_prog + (current_pass_progress * (range_prog / 100.0))
            p_data['job_description'] = desc
            if callable(progress_callback):
                progress_callback(p_data)

        ret_code, _, stderr = utils.run_process(cmd=cmd, duration=info['duration'], total_frames=info.get('nb_frames'), progress_callback=pass_progress_wrapper, process_description=desc, process_holder=process_holder, cancel_flag_func=cancel_flag_func, cwd=ffmpeg_cwd)
    ...
    if total_passes > 1:
        passlogfile_base = os.path.join(job['temp_dir'], f"{config.PASSLOG_FILENAME_BASE}_{job['job_id']}")
...
def run_job(job: Dict, progress_callback: Callable, process_holder: List, cancel_flag_func: Callable) -> Tuple[bool, float]:
    ...
    logger.info(f"--- Preparing Job: {job['job_description_short']} | Mode: {rate_control_mode.upper()} ---")
    ...
    meta['output_name'], meta['resolution_name'] = job['job_description_short'], utils.get_resolution_name(out_w, out_h)
    _, job['output_file'] = utils.generate_output_path(output_dir, meta, preset_conf.get('container', '.mp4'))
    ...
    if preset_conf['video_codec'] == 'libx264':
        ...
        qp_filepath = get_scenecut_qp_filepath(job, config.SCENEDETECT_THRESHOLDS.get(job.get('source_material', 'modern')), cancel_flag_func)
    ...
    if source_material == "animation" and not tuning_str: # Fallback for presets without explicit animation tuning
        tuning_str = tuning_params.get("common")

    x_params_str = ""
    if tuning_str:
        if "{common}" in tuning_str:
            common_params = tuning_params.get("common_x265_params", "") if preset_conf['video_codec'] == 'libx265' else tuning_params.get("common", "")
    ...
    if rate_control_mode == 'crf':
        ...
        logger.info(f"CRF encode selected for '{preset_id}'. Quality: '{quality_level}', CRF Value: {crf_value}")
        ...
        vbv_mult = preset_conf.get('vbv_bufsize_multiplier') or preset_conf.get('bufsize_kb_multiplier', 2.0)
        maxrate_k = int(video_kbps * 1.20)
        bufsize_k = int(maxrate_k * vbv_mult)

        if preset_conf['video_codec'] == 'libx264':
            video_opts.extend(["-maxrate", f"{maxrate_k}k", "-bufsize", f"{bufsize_k}k"])
            logger.info(f"Applying VBV to x264: maxrate {maxrate_k}k, bufsize {bufsize_k}k")
            ...
            logger.info(f"Applying VBV to x265: maxrate {maxrate_k}k, bufsize {bufsize_k}k")
        
        logger.info(f"Calculated ABR for {preset_id}: {video_kbps}k")

    else:
        logger.error(f"Unknown rate control mode '{rate_control_mode}' for preset '{preset_id}'.")
        return False, 0.0
    ...
    if qp_filepath:
        if 'keyint' in x_params_str:
            ...
            logger.info("QP file in use: Removed conflicting 'keyint' from encoder parameters.")
    ...
    if x_params_str:
        ...
        param_key = '-x265-params' if preset_conf['video_codec'] == 'libx265' else '-x264-params'
        video_opts.extend([param_key, x_params_str.lstrip(':')])

    success, elapsed_time = _execute_encode(job, total_passes, video_opts, audio_opts, vf_str, af_str, progress_callback, process_holder, cancel_flag_func)

    if success:
        logger.info(f"--- Finished Job: {job['job_description_short']} ({elapsed_time:.2f}s) ---")
    else:
