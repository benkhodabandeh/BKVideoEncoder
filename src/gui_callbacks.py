# gui_callbacks.py
"""
Contains callback functions for all GUI events in Ben. Khodabandeh Video Encoder.
...
def select_destination_callback(app: 'App'):
    ...
    initial_dir = os.path.dirname(app.input_file_original) if app.input_file_original else os.path.expanduser("~")
    dir_path = filedialog.askdirectory(title="Select Destination Folder", initialdir=initial_dir)
    if dir_path:
...
def preset_button_callback(app: 'App', preset_id: str):
    ...
    if is_fast: target_set = app.selected_fast_presets
    elif not is_standard: target_set = app.selected_workflow_presets
...
def toggle_mb_entry_callback(app: 'App'):
    ...
    if not app.is_video_loaded: return
    current_val = app.target_mb_var.get()
    new_val = ui_components.InputDialog.get_input(app, "Target Size", f"Enter new target size in MB:", initial_value=current_val)
    if new_val is not None:
        try:
            val_float = float(new_val)
            if val_float > 0:
                app.target_mb_var.set(str(round(val_float)))
                gui_updaters.update_mb_button_display(app)
                if "THE_JOB" in app.selected_workflow_presets: app._update_estimates()
            else:
                raise ValueError("Value must be positive.")
        except (ValueError, TypeError):
            app._show_error("Invalid Input", f"'{new_val}' is not a valid positive number.")
...
def crop_mode_callback(app: 'App', choice: str):
    ...
    app._update_estimates()
    gui_updaters.display_preview_image(app, app.preview_stills_paths[app.current_preview_index] if app.current_preview_index != -1 else None)
...
def mux_callback(app: 'App'):
    ...
    video_file = filedialog.askopenfilename(title="Select Video File", filetypes=(("Video Files", "*.mp4 *.mkv"), ("All files", "*.*")), parent=app)
    if not video_file: return
    audio_file = filedialog.askopenfilename(title="Select Audio File", filetypes=(("Audio Files", "*.m4a *.aac *.ac3 *.wav"), ("All files", "*.*")), parent=app)
    if not audio_file: return
    output_file = filedialog.asksaveasfilename(title="Save Muxed File", defaultextension=".mp4", filetypes=(("MP4 Container", "*.mp4"),), parent=app)
...
def audio_only_callback(app: 'App'):
    ...
    input_file = filedialog.askopenfilename(title="Select Input File", filetypes=(("All files", "*.*"),), parent=app)
    if not input_file: return
    output_file = filedialog.asksaveasfilename(title="Save Audio File", defaultextension=".m4a", filetypes=(("AAC Audio", "*.m4a"),), parent=app)
    if not output_file: return
...
def add_to_queue_callback(app: 'App'):
    ...
    if not job_item:
        app._show_error("Preset Error", "Please select at least one preset or workflow.")
        return

    app.batch_queue.append(job_item)
    logger.info(f"Added job '{job_item['metadata'].get('title', 'Untitled')}' to queue. Queue size: {len(app.batch_queue)}")
    gui_panels.update_queue_display(app)

    app._reset_ui_for_new_job()
    app._show_info("Job Queued", f"'{job_item['metadata'].get('title', 'Untitled')}' was added to The Plan.")
...
def develop_callback(app: 'App'):
    ...
    if app.batch_queue:
        ...
        logger.info("Develop started with no jobs in queue. Creating a single job from UI.")
        if single_job := gui_tasks.create_job_item_from_current_state(app):
            jobs_to_run.append(single_job)
        else:
            app._show_error("Not Ready", "Please select at least one preset before developing.")
            return
    else:
        app._show_error("Not Ready", "Please load a video and select presets to develop.")
        return
...
def cancel_callback(app: 'App'):
    if not app.is_processing or app.cancel_requested: return
    if not messagebox.askyesno("Confirm Cancellation", "Are you sure you want to cancel the current process?", parent=app, icon='warning'): return
        ...
        for p in app.process_holder:
            try:
                if p and p.poll() is None:
                    p.terminate()
                    logger.info(f"Sent terminate signal to process PID {p.pid}")
...
def remove_from_queue_callback(app: 'App', job_id: str):
    ...
    logger.info(f"Removed job ID {job_id} from queue. Remaining: {len(app.batch_queue)}")
    gui_panels.update_queue_display(app)
    app._update_ui_state()

def move_queue_item_callback(app: 'App', job_id: str, direction: int):
    try:
        index = next(i for i, job in enumerate(app.batch_queue) if job.get('job_id') == job_id)
...
def reopen_job_callback(app: 'App', job_id: str):
    job_to_load = next((job for job in app.batch_queue if job.get('job_id') == job_id), None)
    if not job_to_load: return
    if app.is_processing:
        app._show_warning("Busy", "Cannot edit a queued job while processing is active.")
        return
    if app.is_video_loaded and app.input_file_original != job_to_load['input_file']:
        if not messagebox.askyesno("Load Different Video?", "This job is for a different video file. Loading it will replace the currently loaded video and its settings. Continue?", parent=app): return

    app.reset_to_load_state(job_to_load['input_file'])
    app.after(50, lambda: _load_job_data(app, job_to_load)) # Defer to allow UI to reset
    remove_from_queue_callback(app, job_id)
    app._show_info("Job Loaded", "The selected job's settings have been loaded for editing.\nIt has been removed from The Plan.")
...
def navigate_preview_callback(app: 'App', direction: int):
    ...
    if num_stills <= 1: return
    app.current_preview_index = (app.current_preview_index + direction) % num_stills
    gui_updaters.display_preview_image(app, app.preview_stills_paths[app.current_preview_index])
    gui_updaters.handle_status_update(app, f"Preview: {app.current_preview_index + 1} of {num_stills}")

def reload_still_callback(app: 'App'):
    """Generates a new random still to replace the currently viewed one."""
    if not app.is_video_loaded or app.current_preview_index < 0: return
    app._start_processing_task(
